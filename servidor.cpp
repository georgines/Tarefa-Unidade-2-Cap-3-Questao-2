...
static void registrar_rotas_http() {
    ...
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/az/ligado", []() { gpio_put(LED_AZUL, true); return ""; }, ServidorHttp::TipoConteudo::HTML);
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/az/desligado", []() { gpio_put(LED_AZUL, false); return ""; }, ServidorHttp::TipoConteudo::HTML);
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/vr/ligado", []() { gpio_put(LED_VERDE, true); return ""; }, ServidorHttp::TipoConteudo::HTML);
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/vr/desligado", []() { gpio_put(LED_VERDE, false); return ""; }, ServidorHttp::TipoConteudo::HTML);
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/vm/ligado", []() { gpio_put(LED_VERMELHO, true); return ""; }, ServidorHttp::TipoConteudo::HTML);
    servidor_http.adicionarRota(ServidorHttp::MetodoHTTP::GET, "/vm/desligado", []() { gpio_put(LED_VERMELHO, false); return ""; }, ServidorHttp::TipoConteudo::HTML);
}

static err_t continuar_envio_http(void* arg, tcp_pcb* pcb, u16_t) {
    auto* estado = static_cast<EstadoEnvioHttp*>(arg);
    if (!pcb || !estado) return ERR_OK;
...
static void enviar_404(tcp_pcb* pcb) {
    ...
    tcp_sent(pcb, [](void* arg, tcp_pcb* p, u16_t) { return tcp_close(p); });
    tcp_output(pcb);
}

static err_t callback_receber_dados(void*, tcp_pcb* pcb, pbuf* p, err_t) {
    if (!pcb || !p) return ERR_OK;
